...
if __name__ == "__main__":
    ...
    parser.add_argument("--report", type=str, help="Generate an aggregate report")
    args = parser.parse_args()

    if args.report == "cmd_executability_by_scenario":
        ...
        # We need to map incident_id back to chaos scenario for this. 
        # But wait, results/main_results.csv doesn't have chaos_scenario column directly.
        # It's generated by llm_engine. Let's look at the columns... 
        # Wait, I'll need to join with metadata or simply read the scenario... 
        # Actually incidents.csv *should* have chaos_scenario if I added it? No, wait. 
        # Let me just compute it via Python pandas reading the metadata files. 
        
        results = []
        for inc_id in df['incident_id'].unique():
            meta_path = f"data/incidents/{inc_id}/metadata.json"
            if os.path.exists(meta_path):
                import json
                with open(meta_path, 'r') as f:
                    meta = json.load(f)
                    scenario = meta.get("chaos_type", "unknown")
                    # update df with scenario
                    df.loc[df['incident_id'] == inc_id, 'scenario'] = scenario
        
        if 'scenario' in df.columns:
            agg = df.groupby('scenario')[['cmd_executability', 'remediation_safe']].mean().reset_index()
            out_path = "figures/cmd_exec_breakdown.csv"
            agg.to_csv(out_path, index=False)
            print(f"✅ Generated {out_path}")
            print(agg.to_string(index=False))
        else:
            print("❌ Could not map incidents to scenarios.")
